---
name: rc_ontology
description: Reading Copilot 지식 온톨로지 생성·갱신 스킬. 사용자의 독서 데이터를 분석해 관심사 그래프와 책 간 연결을 만드는 두뇌 역할입니다. "온톨로지 업데이트해줘", "내 독서 패턴 분석해줘", "어떤 주제에 관심 있는지 정리해줘", 하이라이트 5개 누적, 완독 선언, 7일 이상 미입력 등의 상황에서 트리거합니다. Book-level / User-level / Cross-book 3개 레이어로 분석하고 Google Drive Ontology/ 폴더를 갱신합니다.
---
...
### User-level
- 범위: Books/ 폴더 전체 + Ontology/profile.md
- 입력: 전체 독서 이력, 대화 패턴, 탐색 노드
- 출력:
  - Ontology/profile.md 관심사 그래프 갱신
  - 탐색 노드 추가
  - 공백 영역 감지 ("이 방향으로 이어지는 책이 아직 없어요")

### Cross-book
- 범위: Books/ 폴더 전체 + Ontology/themes.md
- 입력: 전체 책 노트의 하이라이트·ai_ontology_nodes
- 출력:
  - 각 책 노트 ai_connected_books, ai_cross_themes 갱신
  - Ontology/themes.md 연결 노드 추가
  - 연결 근거 코멘트 생성

## 동작 흐름
1. 트리거 감지 → 실행할 레이어 결정
2. 해당 범위 데이터 로드 (Google Drive MCP)
3. 키워드·빈도·감정·반복 패턴 분석
4. Ontology/themes.md 대조 → 기존 노드 병합 또는 신규 추가
5. 책 노트 프론트매터 ai_* 필드 갱신
6. User-level 이상 실행 시 profile.md 갱신
7. ai_last_analyzed 타임스탬프 업데이트

## 노드 기준 변경 시 확산 절차
사용자가 "A 노드를 B로 통합해줘" 요청 시:
1. Ontology/themes.md 노드 정의 변경
2. ai_ontology_nodes에 해당 노드가 있는 모든 책 노트 탐색
