...
## 역할
새 사용자가 Reading Copilot을 처음 사용할 때, **질문 1개**로 모든 초기 셋업을 끝낸다.
...
### 단계 1 — Vault 경로 입력 받기 (유일한 질문)
`AskUserQuestion` 도구로 한 번에 묻는다. 옵션:
- "로컬 폴더 (Recommended)" → `~/Documents/Reading Copilot` 추천
- "iCloud Obsidian Vault" → `~/Library/Mobile Documents/iCloud~md~obsidian/Documents/Reading Copilot` 추천
- "다른 경로 직접 입력" → 사용자가 직접 절대경로 입력

결과로 `vault_root` 절대경로 1개를 확정한다.
경로에 `~`가 포함되면 사용자의 홈 디렉토리로 전개한다.

파일 접근 방식은 묻지 않는다. `file_access: filesystem`을 기본 적용한다 — Read/Write/Edit 도구로 `vault_root`에 직접 읽고 쓴다. Obsidian·MCP 설치는 설정 조건이 아니다. (이미 Obsidian을 쓰는 사용자를 위한 전환 경로는 단계 5 완료 안내에 짧게 남긴다.)
...
생성된 파일 목록과 함께 완료 안내문을 출력한다. 추가 설정은 없다.
...
📌 (선택) 이미 Obsidian을 쓰고 계신가요?
CLAUDE.md의 file_access 값을 "obsidian-mcp"로 바꾸고 "Local REST API" 커뮤니티 플러그인 + mcp-obsidian MCP를 연결하면, Vault를 Obsidian에서 직접 볼 수 있습니다. 지금 당장 필요하지 않으면 건너뛰어도 됩니다.
```
