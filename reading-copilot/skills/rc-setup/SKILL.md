...
**`.obsidian/graph.json`**: Obsidian 그래프 뷰를 열었을 때 책 노트(Contents)와 개념 노드(Ontology/Concepts)가 색으로 바로 구분되도록 path 기반 색상 그룹을 미리 심어둔다.
```json
{
  ...
  "colorGroups": [
    {
      "query": "path:Contents",
      "color": { "a": 1, "rgb": 1127652 }
    },
    {
      "query": "path:Ontology/Concepts",
      "color": { "a": 1, "rgb": 12111871 }
    }
  ]
}
```
`.obsidian/graph.json`이 이미 존재하면(기존 볼트에 설치하는 경우) 덮어쓰지 않고 건너뛴다.
...
- {vault_root}/.obsidian/graph.json (그래프 뷰 색상 프리셋)
