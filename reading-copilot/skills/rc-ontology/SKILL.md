...
온톨로지 분석은 여러 파일을 종합해 개념·연결을 추론하므로 상위 모델(Opus)에서 정확도가 크게 올라간다.
사용자는 `/model` 같은 명령을 잘 모르는 경우가 많으므로, 스킬이 먼저 자연스럽게 권한다.

**동작:**
1. 이 스킬이 트리거되면, 분석을 시작하기 전에 **현재 실행 중인 모델**을 확인한다.
2. 현재 모델이 Opus급 상위 모델이 **아니라면**(예: Haiku·Sonnet 등 속도 우선 모델), 응답 맨 앞에 아래 한 줄을 덧붙인다:
   > 💡 온톨로지 분석은 **Opus** 모델에서 더 정확해요. 상단(또는 설정)의 모델 선택에서 Opus로 바꾸시면 좋아요 — 지금 그대로 진행해도 괜찮아요.
3. 이미 Opus급 상위 모델로 실행 중이면 이 안내를 **생략**한다.
4. 한 세션에서 이미 이 안내를 한 번 했다면 **반복하지 않는다**.
5. 안내 여부와 무관하게 분석은 **그대로 진행**한다. 모델 전환을 강요하거나 실행을 멈추지 않는다.
...
- 권장 모델·전환 안내는 상단 "## 권장 모델 — 실행 시 전환 안내" 참조.
