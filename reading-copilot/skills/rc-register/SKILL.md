---
name: rc-register
description: Reading Copilot 책 등록 스킬. 사용자가 새 책을 독서 목록에 추가하고 싶을 때 사용하세요. 책 표지 이미지를 촬영해서 올리거나, "이 책 등록해줘", "독서 목록에 추가해줘", 책 제목·저자를 언급하며 읽기 시작을 알릴 때 트리거합니다. 서지 정보 수집 및 로컬 Obsidian Vault의 Contents/ 폴더에 노트 파일을 생성하고, 기존 온톨로지와의 첫 연결을 만듭니다.
---
...
- 경로·옵션은 **CLAUDE.md의 `## ⚙ Configuration` 블록**에서 읽는다.
  - Vault 루트: `vault_root` / Contents 폴더: `{vault_root}/{books_dir}` / Ontology: `{vault_root}/{ontology_dir}`
...
3. `{books_dir}` 폴더의 파일 목록 조회 → 이미 등록된 책인지 확인
   - `file_access: obsidian-mcp`: `obsidian_list_files_in_dir` 호출
     - dirpath = `{obsidian_mcp_prefix}/{books_dir}` (후행 슬래시 금지)
     - 예) `dirpath: "reading-copilot-vault/Contents"`
...
5. `{books_dir}/{제목}.md` 신규 생성 (아래 템플릿 사용)
   - obsidian-mcp: `obsidian_append_content` 또는 `obsidian_patch_content`
     - filepath = `{obsidian_mcp_prefix}/{books_dir}/{제목}.md` (후행 슬래시 금지)
...
- LAYER 3 (ai_ 접두사)는 rc_ontology가 채운다. 이 스킬은 빈 상태로 남긴다.
  - 스키마 표준은 rc_ontology 스킬 참조. ai_concepts·ai_connected_contents·ai_cross_themes 는 `"[[위키링크]]"` 형식이며 rc_ontology만 채운다.
